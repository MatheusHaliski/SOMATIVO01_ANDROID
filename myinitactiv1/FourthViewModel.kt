...
class FourthViewModel(private val appDatabase: AppDatabase) : ViewModel() {
    ...
    // Função para popular o banco de dados (caso necessário)
    fun populateDatabase() = liveData<Unit>(Dispatchers.IO) {
        populateTime(appDatabase.timeDao())
        emit(Unit) // Emite um valor vazio após popular o banco
    }
}
